...
export default function App() {
  ...
  const [showInvestmentExplanation, setShowInvestmentExplanation] =
    useState(false);
  const [copiedResult, setCopiedResult] = useState<"emi" | "investment" | null>(
    null,
  );
  const [type, setType] = useState<"emi" | "invest" | "incometax" | "compare">(
    "emi",
  );

  const selectedCountry =
    COUNTRY_MAP[selectedCountryCode] ?? COUNTRY_CONFIGS[0];
  ...
  useEffect(() => {
    ...
    void fetchExchangeRates(selectedCountry.currency, allCurrencies)
      ...
      .catch(() => {
        ...
        setExchangeError(
          "Live FX unavailable. Showing calculator outputs in selected currency only.",
        );
      });
  ...
  const salaryAnnual =
    salaryMode === "monthly"
      ? debouncedInputs.salaryInput * 12
      : debouncedInputs.salaryInput;
  const taxBreakdown = calculateTaxByCountry(
    selectedCountry.code,
    debouncedInputs.taxableIncome,
    taxRegime,
  );
  const netSalary = Math.max(salaryAnnual - taxBreakdown.totalTax, 0);
  const consumptionTaxAmount = calculateConsumptionTax(
    debouncedInputs.consumptionAmount,
    debouncedInputs.consumptionRate,
  );
  const gratuityAmount =
    selectedCountry.code === "IN"
      ? calculateIndiaGratuity(
          debouncedInputs.gratuitySalary,
          debouncedInputs.gratuityYears,
        )
  ...
  const compareTaxA = calculateTaxByCountry(
    compareA.code,
    debouncedInputs.compareIncome,
    compareA.taxRegimes[0] ?? "default",
  );
  const compareTaxB = calculateTaxByCountry(
    compareB.code,
    debouncedInputs.compareIncome,
    compareB.taxRegimes[0] ?? "default",
  );
  ...
  const fxTargets = COUNTRY_CONFIGS.filter(
  ...
  const homepageSeo =
    selectedCountry.code === "IN"
      ? {
          title:
            "Global Financial Calculator | EMI, SIP, Tax & Investment Tools",
          description:
            "Free financial calculator for India. Calculate EMI, SIP returns, tax, and investment growth instantly.",
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white">
      ...
      <header className="sticky top-0 z-20 border-b border-white/10 bg-slate-950/90 backdrop-blur">
        <div className="mx-auto flex w-full max-w-6xl flex-wrap items-center justify-between gap-4 px-4 py-4">
          ...
          <div className="flex items-center gap-3">
            ...
            />
            <div className="rounded-xl border border-white/20 px-3 py-2 text-sm text-slate-200">
              Language: EN
            </div>
          </div>
        </div>
        ...
        <div className="relative mx-auto grid min-h-[58vh] w-full max-w-6xl items-center gap-8 px-4 py-12 md:grid-cols-2">
          <div className="space-y-5">
            ...
            <h1 className="text-4xl font-bold tracking-tight text-white md:text-5xl">
              {selectedCountry.code === "IN"
                ? "All-in-One Financial Calculator India"
                : `${selectedCountry.name} Financial Calculator`}
            </h1>
            <p className="max-w-xl text-slate-200">
              Unified calculators for loans, taxes, salary, investments, and
              benefits. Switch country and currency to update every model in
              real time.
            </p>
            <p className="text-sm text-slate-300">
              Active preset: {selectedCountry.name} loan range{" "}
              {selectedCountry.loanRateRange.min}% to{" "}
              {selectedCountry.loanRateRange.max}%
            </p>
          </div>
          <motion.div
          ...
          >
            ...
            >
              ...
              />
              <path
                d="M138 150h164M220 68v164M162 104c18 12 98 12 116 0M162 196c18-12 98-12 116 0"
                stroke="rgba(148,163,184,0.75)"
                strokeWidth="2"
                fill="none"
              />
              <path
                d="M70 234l56-44 42 20 64-70 44 24 84-98"
      ...
      <main className="mx-auto w-full max-w-6xl px-4 pb-20">
        <div className="flex flex-wrap gap-4 py-6 text-sm text-slate-300">
          <span>
            ...
            {fxTargets
              .map(
                (target) =>
                  `${(exchangeRates[target.currency] ?? 0).toFixed(3)} ${target.currency}`,
              )
              .join(" | ")}
          </span>
          <span>
            {exchangeDate
              ? `Rates date: ${exchangeDate}`
              : "Fetching live rates..."}
          </span>
          {exchangeError ? (
            <span className="text-amber-300">{exchangeError}</span>
          ) : null}
        </div>
        ...
        <div className="p-4 space-y-4 max-w-md">
          ...
          <div className="relative">
            <select
              ...
              className="w-full appearance-none px-4 py-2 pr-10 rounded-xl 
                     border border-gray-300 shadow-sm
                     focus:outline-none focus:ring-2 focus:ring-blue-500 
                     hover:border-gray-400 transition"
            >
              <option value="emi">EMI Calculator</option>
              <option value="invest">Investment Calculator</option>
              <option value="incometax">Income, Tax & Benefits</option>
        ...
        {type === "emi" && (
          <Section
            title="EMI & Loan Calculators"
            subtitle="Universal formulas with country presets and manual APR controls."
          >
            <motion.div
            ...
            >
              <div className="space-y-4 rounded-2xl border border-white/15 bg-white/5 p-4">
                <InfoTooltip
                  label="Universal EMI"
                  description="EMI is the fixed monthly repayment for a loan. APR includes annual interest impact."
                />
                <NumberField
                  label={`Principal (${selectedCountry.currency})`}
                  value={loanPrincipal}
                ...
                {debouncedInputs.loanYears > 5 ? (
                  <p className="text-sm text-slate-300">
                    If you reduce tenure by 5 years, you save{" "}
                    {formatCurrency(
                      emiComparison.saveIfShorter,
                      selectedCountry.currency,
                      selectedCountry.locale,
                    )}
                  </p>
                ...
                <p className="text-sm text-slate-300">
                  If you increase tenure by 5 years, you pay{" "}
                  {formatCurrency(
                    emiComparison.extraIfLonger,
                    selectedCountry.currency,
                    selectedCountry.locale,
                  )}{" "}
                  more
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  {copiedResult === "emi" ? (
                    <span className="text-sm text-cyan-200">Copied</span>
                  ) : null}
                </div>
                {showEmiExplanation ? (
                  <p className="text-sm text-slate-300">
              ...
              <div className="space-y-4 rounded-2xl border border-white/15 bg-white/5 p-4">
                <InfoTooltip
                  label="Home, Car, Personal"
                  description="Country switching updates preset rates. You can always override manually."
                />
                <div className="grid gap-3 text-sm md:grid-cols-3">
                  ...
                  <div className="rounded-xl border border-white/10 p-3">
                    ...
                    <NumberField
                      label="Years"
                      value={personalLoanYears}
                      onChange={setPersonalLoanYears}
                      min={1}
                      step={1}
                      max={8}
                    />
                ...
                <div className="grid gap-3 md:grid-cols-3">
                  ...
                  <NumberField
                    label="Car Principal"
                    value={carLoanPrincipal}
                    onChange={setCarLoanPrincipal}
                    min={1000}
                    max={10000000}
                    step={500}
                  />
                  <NumberField
                    ...
                    value={personalLoanPrincipal}
                    onChange={setPersonalLoanPrincipal}
                    min={1000}
                    max={1000000}
                    step={100}
                  />
            ...
            <div className="space-y-3 text-sm text-slate-300">
              <div className="space-y-2">
                ...
                <p>
                  This EMI calculator India estimate uses your loan amount,
                  annual interest rate, and repayment tenure to show a monthly
                  EMI, total interest, and how changing tenure affects repayment
                  cost.
                </p>
              </div>
              <div className="space-y-2">
                ...
                <p>
                  <strong>What is EMI calculator?</strong> It helps you estimate
                  the fixed monthly repayment for a loan using principal, rate,
                  and tenure.
                </p>
                <p>
                  <strong>How is EMI calculated?</strong> The formula converts
                  annual interest into a monthly rate and spreads repayment
                  across the chosen number of months.
                </p>
              </div>
            </div>
          </Section>
        ...
        {type == "invest" ? (
          <Section
            ...
            subtitle="Term Deposit, Recurring Investment, Simple Interest, and Compound Interest."
          >
            <motion.div
              initial={{ opacity: 0, y: 18 }}
              whileInView={{ opacity: 1, y: 0 }}
            ...
            <div className="grid gap-4 md:grid-cols-2">
              <div className="space-y-3 rounded-2xl border border-white/15 bg-white/5 p-4">
                ...
                <NumberField
                  ...
                  value={termDepositPrincipal}
                  onChange={setTermDepositPrincipal}
                  min={100}
                  max={100000000}
                  step={1000}
                />
                <NumberField
                  label="Annual Rate (%)"
                  value={termDepositRate}
                  onChange={setTermDepositRate}
                  min={0}
                  max={30}
                  step={0.1}
                />
                <NumberField
                  label="Years"
                  value={termDepositYears}
                  onChange={setTermDepositYears}
                  min={1}
                  max={50}
                  step={1}
                />
                <NumberField
                  ...
                  value={termDepositCompounds}
                  onChange={setTermDepositCompounds}
                  step={1}
                  min={1}
                  max={12}
                />
              ...
              <div className="space-y-3 rounded-2xl border border-white/15 bg-white/5 p-4">
                ...
                <label className="flex flex-col gap-2 text-sm text-slate-300">
                  ...
                  >
                    <option value="sip" className="bg-slate-900">
                      SIP
                    </option>
                    <option value="lumpsum" className="bg-slate-900">
                      Lumpsum
                    </option>
                  </select>
                </label>
                {investmentMode === "sip" ? (
                  <NumberField
                    ...
                    onChange={(value) =>
                      setInvestmentForm({ ...investmentForm, monthly: value })
                    }
                    min={100}
                    max={1000000}
                    step={1000}
                ...
                <NumberField
                  ...
                  onChange={(value) =>
                    setInvestmentForm({ ...investmentForm, rate: value })
                  }
                  min={0.1}
                  max={30}
                  step={0.1}
                ...
                <NumberField
                  ...
                  onChange={(value) =>
                    setInvestmentForm({ ...investmentForm, years: value })
                  }
                  min={1}
                  max={40}
                  step={1}
                ...
                {!investmentResult ? (
                  <p className="text-sm text-red-300">
                    Enter positive values to calculate the investment result.
                  </p>
                ) : (
                  <>
                    <p className="text-cyan-200">
                ...
                <div className="flex flex-wrap gap-2">
                  <button
                    type="button"
                    onClick={() =>
                      setShowInvestmentExplanation((current) => !current)
                    }
                    className="rounded-lg border border-white/15 bg-white/5 px-3 py-1.5 text-sm text-slate-200"
                  >
                    Explain Calculation
                  ...
                  {copiedResult === "investment" ? (
                    <span className="text-sm text-cyan-200">Copied</span>
                  ) : null}
                </div>
                {investmentMode === "sip" ? (
                  <p className="text-xs text-slate-400">
                    Inflation check uses a {DEFAULT_SIP_INFLATION.toFixed(1)}%
                    assumption.
                  </p>
                ) : null}
              ...
              <div className="space-y-3 rounded-2xl border border-white/15 bg-white/5 p-4">
                ...
                <NumberField
                  label="Compounds / Year"
                  value={compoundCompounds}
                  onChange={setCompoundCompounds}
                  step={1}
                  min={1}
                  max={12}
                />
            ...
            <div className="space-y-3 text-sm text-slate-300">
              <div className="space-y-2">
                ...
                <p>
                  This SIP calculator India view lets you compare monthly SIP
                  investing with a lumpsum investment so you can understand
                  expected growth, future value, and long-term compounding more
                  clearly.
                </p>
              </div>
              <div className="space-y-2">
                ...
                <p>
                  <strong>How is SIP calculated?</strong> The calculator
                  compounds each monthly investment over the selected duration
                  using the expected annual return.
                </p>
                <p>
                  <strong>Which is better SIP or lumpsum?</strong> SIP can help
                  average entry points over time, while lumpsum can work well
                  when money is available upfront and investment horizon is
                  long.
                </p>
              </div>
            </div>
        ...
        {type === "incometax" ? (
          <Section
            ...
            subtitle="Country-aware tax rules, consumption tax mode, gratuity, and retirement contributions."
          >
             <motion.div
              initial={{ opacity: 0, y: 18 }}
              whileInView={{ opacity: 1, y: 0 }}
            ...
            <div className="grid gap-4 md:grid-cols-2">
              <div className="space-y-3 rounded-2xl border border-white/15 bg-white/5 p-4">
                ...
                <NumberField
                  label={`Gross ${salaryMode === "annual" ? "Annual" : "Monthly"} Salary`}
                  value={salaryInput}
                ...
                />
                <p className="text-sm text-slate-300">
                  Approx annual net after selected tax model
                </p>
                <p className="text-cyan-200">
                  {formatCurrency(
                    netSalary,
              ...
              <div className="space-y-3 rounded-2xl border border-white/15 bg-white/5 p-4">
                <h3 className="text-lg font-semibold">
                  Tax Calculator ({selectedCountry.name})
                </h3>
                <NumberField
                ...
                <p className="text-slate-300">
                  Effective Rate: {formatPercent(taxBreakdown.effectiveRate)}
                </p>
              </div>

              <div className="space-y-3 rounded-2xl border border-white/15 bg-white/5 p-4">
                <h3 className="text-lg font-semibold">
                  {selectedCountry.consumptionTax.label} Calculator
                </h3>
                <InfoTooltip
                  label={selectedCountry.consumptionTax.label}
                  description="GST is used in India, VAT in many regions, Sales Tax in the USA."
                />
                <NumberField
                  label="Net Amount"
                  value={consumptionAmount}
                  onChange={setConsumptionAmount}
                  min={0}
                  max={10000000}
                  step={100}
                />
                <NumberField
                  label={`${selectedCountry.consumptionTax.label} Rate (%)`}
                ...
                <p className="text-slate-300">
                  Gross:{" "}
                  {formatCurrency(
                    debouncedInputs.consumptionAmount + consumptionTaxAmount,
                    selectedCountry.currency,
                    selectedCountry.locale,
                  )}
                </p>
              ...
              <div className="space-y-3 rounded-2xl border border-white/15 bg-white/5 p-4">
                ...
                {!selectedCountry.gratuitySupported ? (
                  <p className="text-sm text-amber-200">
                    Available for India and UAE based on local labor structures.
                  </p>
                ) : (
                  <>
                    <NumberField
                      label="Last Drawn Monthly Basic"
                      value={gratuitySalary}
                      onChange={setGratuitySalary}
                      min={0}
                      max={10000000}
                      step={100}
                    />
              ...
              <div className="space-y-3 rounded-2xl border border-white/15 bg-white/5 p-4 md:col-span-2">
                <h3 className="text-lg font-semibold">
                  ...
                  {selectedCountry.retirementPlanLabel})
                </h3>
                <div className="grid gap-3 md:grid-cols-3">
                  <NumberField
                    label="Annual Income"
                  ...
                  <NumberField
                    label="Employee Rate (%)"
                    value={retirementEmployeeRate}
                    onChange={setRetirementEmployeeRate}
                    min={0}
                    max={30}
                    step={0.1}
                  />
                  <NumberField
                    label="Employer Rate (%)"
                    value={retirementEmployerRate}
                    onChange={setRetirementEmployerRate}
                    min={0}
                    max={20}
                    step={0.1}
                  />
                ...
                <div className="grid gap-2 text-sm text-slate-200 md:grid-cols-3">
                  ...
                  <p>
                    Employer:{" "}
                    {formatCurrency(
                      retirement.employerContribution,
                      selectedCountry.currency,
                      selectedCountry.locale,
                    )}
                  </p>
            ...
            <div className="space-y-3 text-sm text-slate-300">
              <div className="space-y-2">
                ...
                <p>
                  This income tax calculator India section estimates tax from
                  your annual income and supports old vs new regime comparison
                  where available, making it easier to review tax impact and
                  salary planning.
                </p>
              </div>
              <div className="space-y-2">
                ...
                <p>
                  <strong>What is an income tax calculator?</strong> It
                  estimates annual tax liability based on taxable income and the
                  rules applied for the selected country or regime.
                </p>
                <p>
                  <strong>How does old vs new regime comparison help?</strong>{" "}
                  It shows how different deduction rules and slab structures can
                  change the final tax payable.
                </p>
              </div>
            </div>
            </motion.div>
        ...
        {type === "compare" && (
          <Section
            ...
            subtitle="Bonus view: compare monthly EMI and annual tax estimate side by side."
          >
             <motion.div
              initial={{ opacity: 0, y: 18 }}
              whileInView={{ opacity: 1, y: 0 }}
        ...
        <section className="border-t border-white/10 py-8 text-sm text-slate-400">
          <p>
            All calculators support manual overrides. For currencies with live
            FX values, results can be converted from the active base.
          </p>
          <p>
            Example conversion:{" "}
            {formatCurrency(
